...
export class ServiceService {
    ...
    static async create(
        ...
        const thirdPartyTotal =
            data.thirdPartyServices?.reduce(
                (sum, tp) => sum + Number(tp.chargedValue),
                0
            ) || 0

        const finalTotal =
            itemsTotal + thirdPartyTotal > 0
                ? itemsTotal + thirdPartyTotal
    ...
    static async update(
        ...
        const isBeingCompleted =
            newStatus === 'CONCLUIDO' &&
            existingService.status !== 'CONCLUIDO'
