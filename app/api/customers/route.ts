...
import { assertTenantCapacity } from '@/lib/tenant-guard'
...
export const GET = withErrorHandling(async (req: NextRequest) => {
  ...
  const result = await CustomerService.listByTenant(tenantId!, query, session?.user.id)
...
export const POST = withErrorHandling(async (req: NextRequest) => {
  ...
  await assertTenantCapacity(tenantId!, 'customers')

  // Criar cliente
  const customer = await CustomerService.create(data, tenantId!, session!.user.id)
